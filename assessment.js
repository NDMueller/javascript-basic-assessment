...
// #10 Write a function called removeItem that takes in an array of strings, and a string.
 function removeItem(minnesotan,str){
	  arr = minnesotan.filter(function(word){
					return word !== str;
			});
			return arr;
 }
...
function doubleTheFun(zwei){
	if( Number(zwei) ){
		return Number(zwei)*2
	}
	else{
		return zwei+zwei;
	}
...
// return the value from the object that corresponds to the property
	function getValueOfProperty(javaMan,name){
		return javaMan[name]
	}
	var rudy={'name':'dale','hair':'brown'}
...
// the current time as a Date object
	...
	function makeChatMessage(howdy,nicholas){
		var obj ={
			message: howdy,
			author: nicholas,
			timestamp: new Date
...
*/
	...
	function outside(temperature,humdidity,cloudy){
		if(temperature>80 && humdidity>40){
			return "I'm all sweat"
		}
		else if(temperature<40 && cloudy>60){
			return "I have icecicles"
		}
		else if(temperature>80 && humdidity<40 && cloudy<20){
			return "I'm literally in the desert"
		}
		else if(temperature>80 || humdidity>50 || cloudy>50){
